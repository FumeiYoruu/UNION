...
def main():
    ...
    if args.resume_from_checkpoint:
        ...
        if args.use_multi_gpu and torch.cuda.device_count() > 1:
            print(f"Re-wrapping resumed model with DataParallel for {torch.cuda.device_count()} GPUs")
            model = torch.nn.DataParallel(model)
    ...
    if args.merge_weights:
        ...
        model_to_merge = model.module if isinstance(model, torch.nn.DataParallel) else model
