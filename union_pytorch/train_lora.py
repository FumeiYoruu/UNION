...
This version uses DataParallel for multi-GPU training (simpler but slower than DDP).
For better performance, use train_lora_ddl.py with DistributedDataParallel.
...
def get_lora_args():
    ...
    parser = argparse.ArgumentParser(description="PyTorch UNION Training with LoRA (DataParallel)")
...
def main():
    ...
    # Note: Compilation happens BEFORE DataParallel wrapping
    ...
    if args.resume_from_checkpoint:
        ...
        # Compile if requested (before DataParallel wrapping)
        if args.compile_model:
            try:
                torch._dynamo.config.suppress_errors = True
    ...
    if args.merge_weights:
        ...
        # Extract underlying model if wrapped with DataParallel
